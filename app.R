# app.R
# BRFSS Shiny Dashboard - Fixed Summary/Detailed toggle + Year/Location fixes
# No CI plotted (per request). Improved colors.
...
# -----------------------------
# Aggregation engine (supports apply_merges and region_aggregate)
# -----------------------------
aggregate_data <- function(data, breakout_category_id, breakout_col_name, apply_merges = TRUE, region_aggregate = FALSE) {
  # 1. Filter rows for the breakout category (CAT1 is overall - include it when requested)
  ...
  qDf <- qDf %>%
    mutate(
      ...
      Data_value = safe_num(Data_value),
      Sample_Size = safe_num(Sample_Size)
    )
  
  # 3. Apply merges for compressed "Summary (Less)" mode
  ...
  if (region_aggregate) {
    ...
    breakout_category_id <- "REGION"
  }
  
  # 5. Year column detection (some datasets use TimeFrameStart or year)
  year_col <- intersect(names(qDf), c("Year","year","TimeFrameStart"))
  year_col_name <- if (length(year_col) > 0) year_col[1] else NULL
  
  # 6. Decide grouping vars based on breakout_col_name
  if (breakout_category_id == "CAT1" || breakout_category_id == "REGION") {
    # For CAT1 (Overall) usually we want to collapse across Location & Year and group only by Response
    ...
    # Other categories: group by Break_Out and Response
    group_vars <- c("Break_Out", "Response")
  }
  
  # 7. Compute aggregated weighted counts
  plotDf <- qDf %>%
    mutate(
      persons = coalesce(Sample_Size, 0),
      implied_SS = if_else(!is.na(Data_value) & Data_value > 0, persons * 100 / Data_value, NA_real_),
      implied_SS = if_else(is.na(implied_SS), persons, implied_SS)
    ) %>%
    group_by(across(all_of(group_vars))) %>%
    summarise(
      agg_ss = sum(coalesce(implied_SS, 0), na.rm = TRUE),
      agg_persons = sum(coalesce(persons, 0), na.rm = TRUE),
      .groups = "drop"
    ) %>%
    mutate(
      agg_percent = if_else(agg_ss > 0, agg_persons * 100 / agg_ss, NA_real_)
    )
  
  # 8. Factor ordering for common breakouts (age, income)
  if ("Break_Out" %in% names(plotDf)) {
    if (breakout_category_id == "CAT3") { # Age
      age_order <- c("18-24","25-34","35-44","45-54","55-64","65+","21-30","31-40","41-50","51-60","56-65","65-74","75+")
      ...
      inc_order <- c("less than $15,000","$15,000-$24,999","$25,000-$34,999","$35,000-$49,999","$50,000+","$50,000-$99,999","$100,000-$199,999","$200,000+")
      ord <- intersect(inc_order, unique(plotDf$Break_Out))
      if (length(ord) > 0) plotDf$Break_Out <- factor(plotDf$Break_Out, levels = ord, ordered = TRUE)
    }
  }
  
  # 9. If we grouped by a non-standard Year column, rename it to Year for plotting convenience
  if (!is.null(year_col_name) && year_col_name != "Year" && ("Year" %in% group_vars == FALSE)) {
    if (year_col_name %in% names(plotDf)) plotDf <- plotDf %>% rename(Year = all_of(year_col_name))
  }
  ...
  sidebarLayout(
    sidebarPanel(class = "sidebar", width = 3,
                 ...
                 radioButtons("detail_level", "Demographic Detail Level:",
                              ...
                              selected = TRUE),
                 helpText("Summary = merged categories; Detailed = original categories.")
    ),
    mainPanel(class = "main-content", width = 9,
              h3(textOutput("selected_question_title")),
              tabsetPanel(id = "main_tabs",
                          tabPanel("Overall", plotOutput("overall_plot", height = "420px")),
                          tabPanel("By Gender", plotOutput("gender_plot", height = "420px")),
                          tabPanel("By Age Group", plotOutput("age_plot", height = "420px")),
                          tabPanel("By Education", plotOutput("education_plot", height = "420px")),
                          tabPanel("By Income", plotOutput("income_plot", height = "420px")),
                          tabPanel("Temporal (Year)", plotOutput("year_plot", height = "420px")),
...
server <- function(input, output, session) {
  ...
  # aggregated reactives (pass apply_merge where relevant; Overall should ignore apply_merge)
  overall_data <- reactive({ aggregate_data(filtered_q_data(), "CAT1", "Break_Out", apply_merges = FALSE) })
  gender_data  <- reactive({ aggregate_data(filtered_q_data(), "CAT2", "Break_Out", apply_merges = apply_merge()) })
  age_data     <- reactive({ aggregate_data(filtered_q_data(), "CAT3", "Break_Out", apply_merges = apply_merge()) })
  education_data <- reactive({ aggregate_data(filtered_q_data(), "CAT5", "Break_Out", apply_merges = apply_merge()) })
  ...
  base_plot <- function(df, x_var_name, x_axis_title, is_dense = FALSE) {
    ...
    # build palette
    n_responses <- max(3, length(response_levels))
    pal <- RColorBrewer::brewer.pal(min(8,n_responses),"Set2")
    colors <- setNames(pal[1:length(response_levels)], response_levels)
    if ("no" %in% response_levels) colors["no"] <- "#d9d9d9"
    if ("yes" %in% response_levels) colors["yes"] <- "#0072B2"
    
    # prefer consistent stacking: 'no' bottom, 'yes' top when both exist
  ...
  output$location_plot <- renderPlot({
    ...
    if (input$loc_view_type == "regions") {
      ...
      if (input$loc_view_type == "highlight") {
        # rank by the dominant response (most frequent response type)
